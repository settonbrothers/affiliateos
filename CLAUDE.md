...
- **M5 (in progress)**: credit economy is live — every AI action reserves/debits credits (`_shared/credits.ts`; **402** when short, refund on failure), `/billing` + sidebar balance. **Signup is now invite-only** — the signup action requires + redeems an `invite_codes` code (grants bonus credits); generate them at `/admin/invite-codes`. **The DB starts with 0 invite codes, so new signups are blocked until an admin generates one.** Magic-link signup is NOT yet gated. **Stripe + Resend email are NOT built** (keys empty in `.env.local`).
