// Node-side TestKitOrchestrator contract. See decisions/003 — KEEP IN SYNC
// with supabase/functions/_shared/types/testKit.ts (Deno copy). Shape mirrors
// docs/plan/05_AGENT_ROSTER.md §TestKitOrchestrator.
import { z } from 'zod'
...
export const TestKitPayloadSchema = z.object({
  ...
  // Contract is "exactly 3 / at least 5", but we keep bounds slightly lenient
  // so a single off-by-one from the model doesn't force a full retry.
