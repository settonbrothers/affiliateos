...
import type { AdCopyEvidenceResponse } from '@/types/agents/adCopyEvidence'
...
export async function AdCopyView({
  ...
  const env = payload as AdCopyResponse | AdCopyEvidenceResponse | null
  const p = env?.payload
  if (!p) {
    return (
      <p style={{ fontSize: '14px', color: '#B23A24' }}>{t('copyMalformed')}</p>
    )
  }
    ...
    <div className="flex flex-col gap-6" style={{ color: '#1F1B16' }}>
      {'engine_version' in p && (
        <div style={cardStyle}>
          ...
          >
            <h3 style={cardTitleStyle}>{t('copyEvidenceTitle')}</h3>
            <Pill tone={p.output_status === 'ready_for_user' ? 'ok' : 'bad'}>
              {p.output_status}
            </Pill>
          </div>
          ...
          <p style={{ fontSize: '13px', color: '#6B6459' }}>
            {t('copyEvidenceSummary', {
              sources: p.evidence_envelope.sources.length,
              outcomes: p.evidence_envelope.supported_outcomes.length,
        ...
        <div className="flex flex-col gap-2">
          {p.judge.principles.map((pr) => (
            <div
              ...
              style={{ fontSize: '14px' }}
            >
              <Pill tone={pr.verdict === 'pass' ? 'ok' : 'bad'}>
                {pr.verdict}
              </Pill>
      ...
      {p.hooks.length > 0 && (
        <div style={cardStyle}>
          <h3 style={{ ...cardTitleStyle, marginBottom: '14px' }}>
            {t('copyHooksTitle')}
          </h3>
