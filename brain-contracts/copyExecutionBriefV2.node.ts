...
const TasteSelectionSchema = z.object({ selected: z.array(z.record(z.string(), z.unknown())).max(6), excluded: z.array(z.record(z.string(), z.unknown())), selection_policy: z.string().min(1) }).strict()
export const CopyExecutionBriefV2Schema = z.object({
  schema_version: z.literal('copy-execution-brief-v2'), brief_id: z.string().min(1), snapshot_id: z.string().min(1), readiness_status: z.enum(['ready_to_write','needs_avatar','needs_audience','needs_objective','objective_conflict','blocked']),
  ...
  campaign_objective: ObjectiveSchema, audience: AudienceSchema,
  ...
  trace: z.object({ input_sections_consumed: z.array(z.string()), input_sections_rejected: z.array(z.string()), lesson_routes: z.array(z.record(z.string(), z.unknown())), warnings: z.array(z.string()) }).strict()
}).strict()
